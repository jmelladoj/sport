<template>
    <div>
        <titulo-pagina titulo="Usuarios" :accion="false"> 
            <template slot="btn_accion">
                <router-link class="btn btn-success box-shadow btn-icon btn-rounded" to="/usuarios/administrar/"><i class="fa fa-plus"></i> Agregar registro</router-link>
            </template>
        </titulo-pagina>
            ...
            <b-card>
                ...
                <b-table class="my-3" show-empty small striped outlined stacked="sm" :items="items" :fields="fields" :current-page="currentPage" :per-page="perPage" :filter="filter" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" @filtered="onFiltered" >
                    ...
                    <template v-slot:cell(acciones)="row">
                        <router-link v-if="!row.item.deleted_at" class="btn btn-warning btn-rounded btn-xs" :to="{ name: 'administrar_usuarios', params: {id: row.item.id}}"  v-b-tooltip.hover title="Actualizar información del registro"><i class="fa fa-pencil"></i></router-link>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapMutations(['msg_error', 'msg_success']),
